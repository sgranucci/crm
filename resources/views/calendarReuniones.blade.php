...
<div class='box box-default'>
    <div class='box-heading'> </div>
   	<div class='box-body'>
				<div id='calendarioReuniones'></div>
	</div>
</div>
...
@push('scripts')
    ...
    var hoy = new Date().toDateString("Y-m-d");
		...
		$('#calendarioReuniones').fullCalendar({
		header: {
        ...
        @foreach ($agenda as $ag)
        	{
              title: '{{ $ag->nombre. ' - '. $ag->tipo  . ' - '.  $ag->status . ' - ( Dom:' . $ag->domicilio . ' Alt:' . $ag->altura . ' Loc:' . $ag->localidad . ' CP:' . $ag->cp . ')'    }}',
